/**
 * JfMooFlow - Image gallery
 *
 * Dependencies: MooTools 1.4
 *
 * @version			0.2.1
 *
 ...
 * @copyright		Author
 * @docmentation	http://outcut.de/JfMooFlow/Docmentation.html
 */
if( typeof JfMooFlow == 'undefined'){
    var JfMooFlow = new Class({
        ...
        initialize: function(element, options){
            ...
            this.JfMooFlow.addClass('mf').setStyles({
                'overflow':'hidden',
                'background-color':this.options.bgColor,
                'position':'relative',
                'height':this.JfMooFlow.getSize().x*this.options.heightRatio,
                'opacity':0
            });

            if(this.options.useWindowResize) window.addEvent('resize', this.update.bind(this, 'init'));
            if(this.options.useMouseWheel || this.options.useSlider) this.JfMooFlow.addEvent('mousewheel', this.wheelTo.bind(this));
            if(this.options.useKeyInput) document.addEvent('keydown', this.keyTo.bind(this));

            this.getElements(this.JfMooFlow);
        },
        ...
        getJfMooFlowElements: function(key){
            var els = [];
        ...
        addLoader: function(){
            this.JfMooFlow.store('height', this.JfMooFlow.getSize().y);
            this.loader = new Element('div',{
                'class':'loader'
            }).inject(this.JfMooFlow);
            new Fx.Tween(this.JfMooFlow, {
                duration: 800,
        ...
        preloadImg: function(){
            this.loadedImages = new Asset.images(this.getJfMooFlowElements('src'), {
                onComplete: this.loaded.bind(this),
                onProgress: this.createJfMooFlowElement.bind(this)
            });
        },

        createJfMooFlowElement: function(counter, i){
            ...
            obj['div'] = new Element('div').setStyles({
                'position':'absolute',
                'display':'none',
                'height': this.JfMooFlow.getSize().y
            }).inject(this.JfMooFlow);
            obj['con'] = new Element('div').inject(obj['div']).addEvents({
                'mouseover': this.stop.bind(this),
        ...
        createUI: function(){
            ...
            if(this.options.useCaption){
                this.cap = new Element('div').addClass('caption').set('opacity',0).inject(this.JfMooFlow);
            }
            ...
            this.JfMooFlow.adopt(this.nav.adopt(this.autoPlayCon, this.sliderCon, this.resizeCon));
            this.showUI();
        },
        ...
        update: function(e){
            if(e == 'init') return;
            this.oW = this.JfMooFlow.getSize().x;
            this.sz = this.oW * 0.5;
            if(this.options.useSlider){
        ...
        setScreen: function(){
            if(this.isFull = !this.isFull){
                this.holder = new Element('div').inject(this.JfMooFlow,'after');
                this.JfMooFlow.wraps(new Element('div').inject(document.body));
                this.JfMooFlow.setStyles({
                    'position':'fixed',
                    'z-index':'100',
                    'top':'0',
                    'left':'0',
                ...
                this.JfMooFlow.wraps(this.holder);
                ...
                this.JfMooFlow.setStyles({
                    ...
                    'left':'',
                    'width':'',
                    'height':this.JfMooFlow.retrieve('height')
                });
    ...
    /* JfMooFlow Viewer (extra JS JfMooFlowViewer.js)
 ...
 */
    JfMooFlow.implement({
        ...
        createIconTip: function(){
            ...
            if(!$chk(this.icon)) {
                this.icon = new Element('a').addClass('show').setStyles({
                    'display':'none',
                    'opacity':'0'
                }).inject(this.JfMooFlow);
            }
            ...
            if(!$chk(this.tip)){
                this.tip = new Element('div').addClass('tooltip').setStyles({
                    'display':'none',
                    'opacity':'0'
                }).inject(this.JfMooFlow);
            }
        ...
        showTip: function(cur){
            ...
            this.tipFx = new Fx.Morph(this.tip,{
                ...
                'opacity': 0.8,
                'top': this.icon.getCoordinates().top-this.tip.getSize().y-100
            });
        },
