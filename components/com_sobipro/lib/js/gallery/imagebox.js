/*
	JfImageBoxAdvanced v1.3.4b - The ultimate extension of Slimbox and JfImageBox; an all-media script
	updated 2010.09.21
...
(function() {
    // Global variables, accessible to JfImageBox only
    var options, images, activeImage, prevImage, nextImage, top, mTop, left, mLeft, winWidth, winHeight, fx, preload, preloadPrev = new Image(), preloadNext = new Image(), foxfix = false, iefix = false,
    // DOM elements
    overlay, center, image, bottom, captionSplit, title, caption, prevLink, number, nextLink,
    // JfImageBox specific vars
    URL, WH, WHL, elrel, mediaWidth, mediaHeight, mediaType = "none", mediaSplit, mediaId = "JfImageBox", mediaFmt, margin;

    /*	Initialization	*/

    window.addEvent("domready", function() {
        // Create and append the JfImageBox HTML code at the bottom of the document
    ...
    JfImageBox = {
        ...
        open: function(_images, startImage, _options) {
            options = $extend({
                ...
                playerpath: 'plugins/content/pophup/player/NonverBlaster.swf',	// Path to NonverBlaster.swf
    ...
    Element.implement({
        JfImageBox: function(_options, linkMapper) {
            $$(this).JfImageBox(_options, linkMapper);	// The processing of a single element is similar to the processing of a collection with a single element
    ...
    Elements.implement({
        /*
			options:	Optional options object, see JfImageBox.open()
		...
		*/
        JfImageBox: function(_options, linkMapper, linksFilter) {
            ...
            links.removeEvents("click").addEvent("click", function() {
                ...
                return JfImageBox.open(filteredLinks.map(linkMapper), filteredHrefs.indexOf(this.toString()), _options);
            });
        ...
        if (Browser.Engine.gecko) {
            ["object", window.ie ? "select" : "embed"].forEach(function(tag) {
                Array.forEach(document.getElementsByTagName(tag), function(el) {
                    if (open) el._JfImageBox = el.style.visibility;
                    el.style.visibility = open ? "hidden" : el._JfImageBox;
                });
            });
        }
    ...
    function changeImage(imageIndex) {
        if (imageIndex >= 0) {
            ...
            /*	JfImageBoxAdvanced link formatting and media support	*/
            ...
            if (URL.match(/\.gif|\.jpg|\.jpeg|\.png|twitpic\.com/i) || mediaType == 'image') {
                mediaType = 'img';
                preload = Asset.image(URL, {
                    onLoad: startEffect,
                    onError: function(err){
                        alert('Failed loading image: ' + err.src);
                    }
                });
            // FLV, MP4
            }
    ...
    function startEffect() {
        ...
        //preload;
            ...
            image.set('html', '<div id="mbError"><b>Error</b><br/>A file type error has occoured, please visit <a href="iaian7.com/webcode/JfImageBoxAdvanced" title="JfImageBoxAdvanced" target="_new">iaian7.com</a> or contact the website author for more information.</div>');
            mediaWidth = options.defaultWidth;
            mediaHeight = options.defaultHeight;
        }
        image.setStyles({
...
JfImageBox.scanPage = function() {
    //	$$('#mb_').each(function(hide) { hide.set('display', 'none'); });
    var links = $$("a").filter(function(el) {
        return el.rel && el.rel.test(/^imagebox/i);
    });
    $$(links).JfImageBox({/* Put custom options here */}, null, function(el) {
        var rel0 = this.rel.replace(/[[]|]/gi," ");
        var relsize = rel0.split(" ");
        return (this == el) || ((this.rel.length > 8) && el.rel.match(relsize[1]));
    });
};
window.addEvent("domready", JfImageBox.scanPage);