...
class adagencyModeladagencyAdcode extends JModel {
	var $_licenses;
	var $_license;
	var $_id = null;
	var $_total = 0;
	...
	function store () {
		...
		$adcodu = preg_replace("/(href((\s+)?)=((\s+)?)\")\b((http(s?):\/\/)|(www\.)|(ftp:\/\/))([\w\.]+)([-~\/\w+\.-?\&\=]+)\b/i","href=\"ad_url",$adcodu);
		$adcodu = str_replace('ad_url/','ad_url',$adcodu);
		$content = $adcodu;
		...
		if (isset($data['adv_cmp'])) {
			foreach ($data['adv_cmp'] as $val) {
				...
				if ( ($val) && (
					...
					$query = "INSERT INTO `#__ad_agency_campaign_banner` (`campaign_id` ,`banner_id` ,`relative_weighting` ,`thumb`, `zone`) VALUES ('".intval($val)."', '".intval($idi)."', '100', NULL, ".addslashes(trim($data['czones'][$val])).");";
					$db->setQuery($query);
					$db->query();
				}
			}
		}
		$sql = "UPDATE #__ad_agency_banners SET `ad_code` = '".$db->getEscaped($content)."' WHERE `id`='".intval($where)."'";
		$db->setQuery($sql);
		$db->query();
