...
class adagencyModeladagencyTextlink extends JModel {
	var $_licenses;
	var $_license;
	var $_id = null;
	var $_total = 0;
	...
	function getlistAds () {
		if (empty ($this->_licenses)) {
			$sql = "select a.aid AS advertiser_id2, a.company AS advertiser, concat(width,'x',height) AS size_type,
			b.*,m.id mid, m.title zone_name
			...
			LEFT JOIN #__modules AS m ON m.id=b.zone
			WHERE 1=1
			GROUP BY b.id
			ORDER BY b.id DESC
			";
