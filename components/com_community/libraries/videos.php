...
class CVideos extends JObject {
    ...
    public function runConvert() {
        ...
        /* we don't use getPendingVideo to prevent caching */
        //return $this->getVideos(array('status' => 'pending'));
        // public function getVideos($filters = array(), $tableBind = false, $limitSetting = true, $useCache = true) {
        $videos = $model->getVideos(array('status' => 'pending'), false, true, false);
    ...
    public function createVideoThumb($videoFile, $thumbFile, $videoFrame, $thumbSize = '128x96') {
        ...
        if (JFile::exists($thumbFullPath) && (filesize($thumbFullPath) > 0)) {
            ...
            if ($this->debug) {
                ...
                echo '<pre>' . $cmdOut . '</pre>';
                if (!$cmdOut) {
                    echo '<pre>Check video thumb folder\'s permission.</pre>';
                }
            }
            return false;
        }
